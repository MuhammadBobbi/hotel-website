    ...
    dataType: {
      async handler(a, b){
        const response = await this.$api.badasoEntity.read({
          slug: 'room',
          id: this.dataType.dataRows[1].value
        });
        console.log( this.dataType.dataRows, response.data.tglCheckout, "test")
        this.dataType.dataRows[2].value = response.data.name
      },
      deep: true
    }
  ...
  methods: {
    submitForm() {
      ...
      this.$api.badasoEntity
        ...
        .then((response) => {
          this.$closeLoader();
          // ids = response.data.noKamar
          console.log(response.data, 'coba')
          this.$api.badasoEntity.read({
              slug: 'room',
              id: response.data.noKamar
            })
            ...
            .then((responses) => {
              ...
              this.$api.badasoEntity.edit({
                ...
                data: {
                  ...
                  status: 0,
                  tipeKamar: responses.data.tipeKamar,
                  noKamar: responses.data.noKamar,
        ...
        .then((response) => {
          ...
          .then((response) => {
            ...
            let data = response.data.data
              this.relationData.room = []
            data.forEach(element => {
                if(element.status !== 0){
                  console.log('kosong',this.relationData.room)
                  this.relationData.room.push(
                    {
                      id: element.id,
                      label: element.noKamar,
                      noKamar: element.noKamar,
                      value: element.id    
                    }
                  )
                }
